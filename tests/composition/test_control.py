...
class TestControlSpecification:
    ...
    def test_deferred_init(self):
        ...
        comp.add_controller(controller=pnl.OptimizationControlMechanism(
                model=comp,
                state=[Input.input_port, reward.input_port],
    ...
    def test_partial_deferred_init(self):
        ...
        ocomp.add_controller(
            pnl.OptimizationControlMechanism(
                model=ocomp,
                state=[initial_node_a.input_port,
    ...
    def test_model_assignement_as_controller_and_replacement(self):
        mech = pnl.ProcessingMechanism()
        comp = pnl.Composition(name='comp',
                           pathways=[mech],
                           controller=pnl.OptimizationControlMechanism(model=None,
        ...
        new_ocm = pnl.OptimizationControlMechanism(model=None,
...
class TestControlMechanisms:
    ...
    def test_lvoc(self):
        ...
        c._analyze_graph()
        lvoc = pnl.OptimizationControlMechanism(model=pnl.RegressionCFA,
                                                state=[m1.input_ports[0], m1.input_ports[1], m2.input_port],
                                                objective_mechanism=pnl.ObjectiveMechanism(
                                                    monitor=[m1, m2]),
    ...
    def test_lvoc_both_predictors_specs(self):
        ...
        c._analyze_graph()
        lvoc = pnl.OptimizationControlMechanism(model=pnl.RegressionCFA,
                                                state=[m1.input_ports[0], m1.input_ports[1], m2.input_port, m2],
                                                objective_mechanism=pnl.ObjectiveMechanism(
                                                    monitor=[m1, m2]),
                                                function=pnl.GridSearch(max_iterations=1),
                                                control_signals=[
    ...
    def test_two_tier_ocm(self):
        ...
        stabilityFlexibility.add_controller(
            pnl.OptimizationControlMechanism(model=stabilityFlexibility,
                                             state=[taskLayer.input_port,
        ...
        outerComposition.add_controller(
            pnl.OptimizationControlMechanism(model=stabilityFlexibility,
                                             state=[taskLayer.input_port, stimulusInfo.input_port],
                                             state_feature_function=pnl.Buffer(history=2),
...
class TestModelBasedOptimizationControlMechanisms:
    ...
    def test_model_based_ocm_after(self, benchmark, mode):
        ...
        ocm = pnl.OptimizationControlMechanism(model=comp,
                                               state=[A.input_port],
                                               objective_mechanism=objective_mech,
                                               function=pnl.GridSearch(),
    ...
    def test_stability_flexibility_susan_and_sebastian(self):
        ...
        metaController = pnl.OptimizationControlMechanism(model=stabilityFlexibility,
                                                          state=[taskLayer.input_port, stimulusInfo.input_port],
    ...
    def test_input_CIM_assignment(self, comp_mode):
        ...
        comp.add_controller(
            pnl.OptimizationControlMechanism(
                model=comp,
                state=[input_b.input_port, input_a.input_port],
                name="Controller",
                objective_mechanism=pnl.ObjectiveMechanism(
