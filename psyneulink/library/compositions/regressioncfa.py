...
assigned to the state field of the `prediction_vector <RegressorCFA.prediction_vector>`, and the `control_allocation
...
class RegressionCFA(CompositionFunctionApproximator):
    ...
    def adapt(self, state_feature_values, control_allocation, net_outcome, context=None):
        ...
        **net_outcome** from **state_feature_values** and **control_allocation**.
        ...
        if previous_state is not None:
            ...
            prediction_vector.update_vector(control_allocation, state_feature_values, context)
    ...
    def evaluate(self, state_feature_values, control_allocation, num_estimates, context):
    ...
    class PredictionVector():
        ...
        state_feature_values : 2d nparray
            arrays of state to assign as the `PV.F` term of `terms <PredictionVector.terms>`.
        ...
        def __init__(self, state_feature_values, control_signals, specified_terms):
            ...
            self.terms[F] = f = state_feature_values
        ...
        # FIX: 11/9/19 LOCALLY MANAGE STATEFULNESS OF ControlSignals AND costs
        def update_vector(self, variable, state_feature_values=None, context=None):
            ...
            if state_feature_values is not None:
                self.terms[PV.F.value] = np.array(state_feature_values)
            # FIX: 11/9/19 LOCALLY MANAGE STATEFULNESS OF ControlSignals AND costs
        ...
        def compute_terms(self, control_allocation, context=None):
            ...
            # No need to calculate state, so just get values
